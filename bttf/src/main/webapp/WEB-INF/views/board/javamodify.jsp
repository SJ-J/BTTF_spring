   ...
   <c:set var="board" value="${requestScope.board }"/>
                ...
                <div class="col-sm-12 sm-margin-b-2" style="margin-bottom: 20px;">
                    <div class="wow fadeInLeft" data-wow-duration=".3" data-wow-delay=".3s">
                        <div class="my_box" data-height="height">
                            <!-- Start write Form -->
							<form name = "javamodify" action="${pageContext.request.contextPath }/board/javamodify" method="post">
								<input type="hidden" name="post_id" value="${javaview.post_id }">
								<div class="col-auto">
									<label for="subject">제목</label>
									<input id="subject" name="post_subject" class="form-control margin-b-50" type="text" placeholder="제목" value="${javaview.post_subject }">
									<textarea id="editor" name="post_contents" class="form-control" placeholder="내용을 입력해 주세요." 
									style="height: 650px; resize: none;">${javaview.post_contents }</textarea>
                                </div> 
                            <a href="javascript:document.javamodify.submit()" class="btn btn-info mt-4" id="writesuccess">작성완료</a>
                        	<%-- <a href="${pageContext.request.contextPath }/board/javalist" class="btn btn-default mt-4" id="edit" type="submit">글 목록</a> --%>
                        	<a class="btn btn-primary mt-4" id="list" onclick="confirm_backlist_java()">목록</a>
                            </form>
                            <!-- End write Form -->
                        </div>
                    </div>
	...
	<script>
    	...
    	function updateReply( post_id ){
    		if (true){
    			document.edit.action = "${pageContext.request.contextPath}/pages/javaBoardView.js?post_id=" +  post_id;
